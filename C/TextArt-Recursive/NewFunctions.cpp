...
void menuTwo(char canvas[][MAXCOLS], char backupCanvas[][MAXCOLS], bool& animate, char& choice)
{
	Point pos1, pos2;
	DrawPoint tree;
	char input, oldChar, condition;
	...
	do {
		...
		gotoxy(MAXROWS + 1, 0);
		if (animate)
			printf("<A>nimate: Y / <F>ill / <L>ine / <B>ox / <N>ested Boxes / <T>ree / <M>ain Menu: ");
		else if (!animate)
			printf("<A>nimate: N / <F>ill / <L>ine / <B>ox / <N>ested Boxes / <T>ree / <M>ain Menu: ");
		...
		else if (choice == 'F' || choice == 'f') 
		{
			...
			do
			{
				...
				{
					oldChar = canvas[row][col];
					copyCanvas(backupCanvas, canvas);
					fillRecursive(canvas, row, col, oldChar, input, animate);
					displayCanvas(canvas);
					gotoxy(MAXROWS + 1, 0);
					printf("Enter character to fill with from current location / <ESC> to cancel");
					gotoxy(row, col);
		...
		else if (choice == 'L' || choice == 'l') 
		{
			...
			if (condition != ESC)
				drawLine(canvas, pos1, pos2, animate);
		...
		else if (choice == 'B' || choice == 'b') 
		{
			...
			if (condition != ESC) {
				copyCanvas(backupCanvas, canvas);
				drawBox(canvas, pos1, height, animate);
			}
		}
		else if (choice == 'N' || choice == 'n')
		{
			...
			if (condition != ESC) {
				copyCanvas(backupCanvas, canvas);
				drawBoxesRecursive(canvas, pos1, height, animate);
			}
		}
		else if (choice == 'T' || choice == 't') 
		{
			...
			{
				copyCanvas(backupCanvas, canvas);
				treeRecursive(canvas, tree, height, startAngle, branchAngle, animate);
			}
		}
